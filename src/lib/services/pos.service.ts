...
export const posService = {
    ...
    async buscarProductos(q: string, soloConStock = true): Promise<ProductoPOS[]> {
        ...
        const response = await apiMilenium.get<ApiResponse<ProductoPOS[]>>(`/productos/pos/buscar?${params}`);
        return response.data;
    },

    async buscarPorBarcode(codigo: string): Promise<ProductoPOS> {
        const response = await apiMilenium.get<ApiResponse<ProductoPOS>>(`/productos/pos/barcode/${encodeURIComponent(codigo)}`);
        return response.data;
    },
    ...
    async validarCantidad(
        ...
        const response = await apiMilenium.get<ApiResponse<{ valido: boolean; cantidad_maxima: number }>>(`/productos/pos/${id}/validar-cantidad?${params}`);
        return response.data;
    },
    ...
    async getResumenSesion(id: string): Promise<ResumenVentasSesion> {
        const response = await apiMilenium.get<ApiResponse<ResumenVentasSesion>>(`/ventas/sesion/${id}/resumen`);
        return response.data;
    },

    async cerrarCaja(id: string, data: CerrarCajaDto): Promise<ResumenCierre> {
        const response = await apiMilenium.patch<ApiResponse<ResumenCierre>>(`/session_caja/${id}/cerrar`, data);
        return response.data;
    },
    ...
    async buscarClientes(q: string): Promise<ClientePOS[]> {
        ...
        const response = await apiMilenium.get<{ status: string; data: { clientes: ClientePOS[] } }>(`/clientes?${params}`);
